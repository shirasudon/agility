...
export function room(state = roomInitialState, action){
    switch(action.type){
        ...
        case RECEIVE_MESSAGE:
            ...
            return state.merge({ 
                oldestMessageTimestamp: Math.min(createdAt, state.get("oldestMessageTimestamp")),
            })
...
export function rooms(state = Immutable.fromJS(
    {
        byId: {}, 
        all: []
    }
    ...
    switch(action.type){
        case RECEIVE_ROOM: {
            if ( !state.get("all").includes(action.id) ) { // TODO: make `all` a set
                state = state.setIn(
                    ["byId", action.id], 
    ...
    switch (action.type) {
        case RECEIVE_MESSAGE: {
            ...
            if (!state.get("all").includes(id)) {
                ...
                state = state.updateIn(["byRoomId", roomId], Immutable.List(), messages => messages.push(action.message.id))
        ...
        case RECEIVE_MESSAGE_READ: {
            ...
            if (!state.getIn(["byId", messageId, "readBy"]).includes(userId)) {
                state.updateIn(["byId", messageId, "readBy"], readBy => readBy.push(userId))
            }
            return state
        }
